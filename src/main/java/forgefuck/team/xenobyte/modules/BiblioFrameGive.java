    ...
    @Override public void onPerform(PerformSource src) {
        ...
        try {
            if (Class.forName("jds.bibliocraft.tileentities.TileEntityMapFrame").isInstance(checkTile)) {
                ...
                widgetMessage(String.format(lang.get("map frame block saved by coords") + " [%s, %s, %s]", coords[0], coords[1], coords[2]), WidgetMode.INFO);
            } else if (coords != null) {
                ...
                if (frameTile != null) {
                    ...
                    if (frameTag.getByte("hasMap") == 1) {
                        ...
                        widgetMessage(lang.get("no found map in map frame!"), WidgetMode.FAIL);
                    }
                } else {
                    widgetMessage(lang.get("map frame block not found within client!"), WidgetMode.FAIL);
                }
            } else {
                widgetMessage(lang.get("map frame block not set! (set by look and press keybind)"), WidgetMode.FAIL);
            }
        } catch(Exception e) {}
    }
    
    @Override public String moduleDesc() {
        return lang.get("Issuing an item to hand using a map frame");
    }
