...
public class Ceiling extends CheatModule {
    ...
    @Override public void onPerform(PerformSource src) {
        ...
        if (isHeight(y)) {
            if (isLava(x, y, z)) {
                widgetMessage("- " + lang.get("lava on the way!"), WidgetMode.FAIL);
            } else {
                utils.verticalTeleport(y + 1, true);
            }
        } else {
            widgetMessage("- " + lang.get("no matching block found"), WidgetMode.FAIL);
        }
    }
    
    @Override public String moduleDesc() {
        return lang.get("Teleportation along the Y axis through blocks up/down by sight");
    }
